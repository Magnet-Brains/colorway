	...
	class Colorway_Admin_Settings {
		...
		public static function required_plugins_install() {
			...
			$plugins = array(
				...
				array(
					'name'    => 'elementor',
					'path'    => 'https://downloads.wordpress.org/plugin/elementor.2.7.5.zip',
				...
				if ( version_compare( $colorway_global_options['colorway-addon-auto-version'], '1.2.1' ) < 0 ) {
					...
					$message = sprintf(
						/* translators: %1$1s: Addon Name, %2$2s: Minimum Required version of the Colorway Addon */
							__( 'Update to the latest version of %1$2s to make changes in settings below.', 'colorway' ),
						( ! empty( $update_colorway_addon_link ) ) ? '<a href=' . esc_url( $update_colorway_addon_link ) . ' target="_blank" rel="noopener">' . $colorway_addon_name . '</a>' : $colorway_addon_name
					);
